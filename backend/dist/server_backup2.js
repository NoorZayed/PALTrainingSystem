...
app.post("/api/supervisor/submit-report", (req, res) => {
    const { student_id, course_subject, internship_type, month, year, full_name, supervisor_name, date_from, date_to, time_from, time_to, supervisor_comments } = req.body;
...
app.put("/api/applications/:applicationId/accept", (req, res) => {
    const { applicationId } = req.params;
    const sql = `UPDATE applications SET status = 'accepted' WHERE application_id = ?`;
    db_1.default.query(sql, [applicationId], (err, result) => {
        if (err) {
...
app.put("/api/applications/:applicationId/reject", (req, res) => {
    const { applicationId } = req.params;
